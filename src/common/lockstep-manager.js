...
export default class LockStepManager {
    ...
    lockStepTurn (processActionsCallback) {
        ...
        if (readyForNextTurn) {
            ...
            if (this.lockStepTurnID >= 3) {
                ...
                for (let i = this.playerIdToProcessFirst; i < this.pendingActions.currentActions.length; i++) {
                    const action = this.pendingActions.currentActions[i];
                ...
                if (commands.length > 0) {
                    ...
                    if (this.playerIdToProcessFirst >= this.pendingActions.currentActions.length) {
                        this.playerIdToProcessFirst = 0;
                    }
                }
    ...
    readyForNextTurn () {
        return (this.confirmedActions.readyForNextTurn(this.lockStepTurnID) &&
