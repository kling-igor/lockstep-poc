...
export default class GameController {
    ...
    animation(time) {
        ...
        this.gameView.render(drawInterpolationFactor);
        ...
        if (button === 2) {
            ...
            if (uids.length > 0) {
                this.lockStepManager.addCommand({
                    uids: uids,
                    command: 'move',
                    details: {// TODO: экранная position должна учитывать смещение карты (см код создания Mouse в сцене)
                        position:{x:position.x / Game.CELL_SIZE | 0,
                                  y:position.y / Game.CELL_SIZE | 0
                        }
                    }
                });
            }
